...
class D1XComboStringCriterion(D1xOption):
    base_entry = '"field":"%s","value":"%s","critClass":"forescout.plugin.dot1x.default_policy.D1XComboStringCriterion","selected":"%s"'
...
class D1XStringCriterion(D1xOption):
    ...
    def return_admission_rule_entry(self, rule_dict):
        ...
        return self.map[rule_dict['fields'][0].lower().replace(" ", "")] % (
            rule_dict['fields'][1], rule_dict['rule_name'], rule_dict['fields'][1])
...
class Context:
    ...
    string_set = ['User-Name', 'Tunneled-User-Name', 'Calling-Station-ID', 'Called-Station-ID', 'SSID',
                  'NAS-Identifier', 'NAS-IP-Address', 'NAS-IPv6-Address', 'Certificate-Issuer',
                  'Certificate-Common-Name', 'Certificate-Subject', 'Certificate-Subject-Alternate-Name-Email',
                  'Certificate-Subject-Alternative-Name', 'Certificate-EAP-TLS-Certificate-Template', 'MAR Comment']
